...
export async function POST(req: Request) {
  try {
    ...
    // High limit to capture all items
    ...
    const prompt = `
      ...
      INSTRUCTIONS:
      1. **Highlights:** Identify ALL key themes. Do not limit the count.
      2. **Categorization:** You MUST split the remaining items into exactly THREE distinct cards:
         - **Connectors**
         - **Customer & Access Management**
         - **Routing & Core Improvements**
      3. **Coverage:** Be COMPREHENSIVE. Include every significant update.
        ...
        <div class="rounded-xl bg-gradient-to-br from-indigo-50/50 to-white/50 dark:from-slate-800 dark:to-slate-900 border border-indigo-100 dark:border-slate-700 p-6 shadow-sm">
           ...
           <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              <div class="space-y-1">
                 ...
                 <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Summary text.</p>
              </div>
           </div>
        </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4">
                        ...
                        <div class="flex-1 space-y-2">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <a href="..." target="_blank" class="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-md bg-indigo-50 text-indigo-600 border border-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-800 text-[10px] font-mono font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">#123</a>
                            </div>
                        </div>
                    </div>
                </div>
